...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void _sendMessage({String? text, List<String>? imageUrls}) async {
    if ((text != null && text.isNotEmpty) ||
      ...
      try {
        // สร้าง Chat Room ID
        String chatRoomId = _createChatRoomId(widget.userId, widget.ownerId);

        // ส่งข้อความไปยังห้องแชทที่สร้างขึ้น
        await _messagesCollection.doc(chatRoomId).collection('messages').add({
          'text': text ?? '',
          'createdAt': Timestamp.now(),
          'senderId': currentUserId,
          'imageUrls': imageUrls ?? [],
          'chatRoomId': chatRoomId, // เก็บ chatRoomId
        });
  ...
  String _createChatRoomId(String userId, String ownerId) {
    ...
    return digest.toString(); // ใช้ hash เป็น ID
  }
